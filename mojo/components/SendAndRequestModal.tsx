...
import { apiURL, useAuth } from 'providers/auth-provider';
...
export default function SendAndRequestModal({
  ...
  const handleSend = () => {
    axios
      .post(
        `${apiURL}/payments/send`,
        {
          send_username: otherPartyDetails.email,
          amount: amount,
        },
        {
          headers: {
  ...
  const handleRequest = () => {
    axios
      .post(
        `${apiURL}/payments/send`,
        {
          request_username: otherPartyDetails.email,
          amount: amount,
        },
        {
          headers: {
  ...
  return (
    <View className="bg-background rel fixed inset-0 top-0 right-0 bottom-0 left-0 z-50 h-screen w-screen items-center justify-center">
      <View className="bg-background border-border relative mx-4 w-[90%] rounded-xl border p-8">
        ...
        <View className="flex flex-row items-center justify-center">
          <Pressable
            onPress={type === 'send' ? handleSend : handleRequest}
            className={cn(
